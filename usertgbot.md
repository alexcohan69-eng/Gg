...
- [ ] **Phase 0 — Bot & env setup**: create `@WebBanaiBot` via `@BotFather`, obtain `TELEGRAM_BOT_TOKEN` + `TELEGRAM_BOT_USERNAME`; generate `TELEGRAM_WEBHOOK_SECRET` and `TELEGRAM_TOKEN_ENCRYPTION_KEY` (32-byte random values); add all four to project env vars via `SystemAction`; register the builtin webhook once Phase 3's route exists.
- [ ] **Phase 1 — Data & crypto foundation**: `telegramLinks` schema + bootstrap script update + run against live DB; `lib/telegram/crypto.ts`; `lib/telegram/client.ts`.
- [ ] **Phase 2 — Link lifecycle**: `lib/telegram/links.ts` (start/complete/verify/unlink for both bot kinds).
- [ ] **Phase 3 — Webhook routes**: `app/api/telegram/webhook/builtin`, `app/api/telegram/webhook/custom/[secret]`, secret-token validation, `/start` + verification-code handling.
- [ ] **Phase 4 — Command dispatcher (read + core actions)**: `lib/telegram/commands.ts` covering link lifecycle, profile, posts, engagement, social graph, notifications, search, and list/view/delete for services/portfolio/testimonials.
- [ ] **Phase 5 — Two-way direct messages**: `notifyNewMessage` hook in `app/actions/messages.ts`, `/inbox`, `/dm`, plain-text reply routing via `activeConversationId`, inline "Reply" buttons.
- [ ] **Phase 6 — Settings UI**: `app/actions/telegram.ts`; `/settings/telegram` page (both linking flows, verification, unlink); new card on `/settings/page.tsx`.
- [ ] **Phase 7 — Commands reference page**: `/telegram-commands` static docs page.
- [ ] **Phase 8 — Verification**: manual pass — link the builtin bot end-to-end (deep link → code → verified), run one command from each group in the Section 6 table, link a second test account to a real BYO bot token and confirm its webhook only accepts requests with the matching secret, send a DM from the web app and confirm a Telegram push arrives, reply from Telegram and confirm it lands back in the web conversation, then unlink both and confirm the webhooks are deleted and commands are refused afterward.
