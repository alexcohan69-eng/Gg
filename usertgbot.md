...
- [ ] **Phase 0 — Bot & env setup**: create `@WebBanaiBot` via `@BotFather`, obtain `TELEGRAM_BOT_TOKEN` + `TELEGRAM_BOT_USERNAME`; generate `TELEGRAM_WEBHOOK_SECRET` and `TELEGRAM_TOKEN_ENCRYPTION_KEY` (32-byte random values); add all four to project env vars via `SystemAction`; register the builtin webhook (`setWebhook` against `TELEGRAM_BOT_TOKEN` pointing at `/api/telegram/webhook/builtin`, `secret_token: TELEGRAM_WEBHOOK_SECRET`) once that's done. **Not started — the only remaining step.** Everything below is built and works end-to-end for the BYO-bot path already; the built-in bot path just needs these four env vars to go live (verified live in the browser: attempting to link the built-in bot currently surfaces a clear "Missing TELEGRAM_BOT_USERNAME" error instead of crashing).
